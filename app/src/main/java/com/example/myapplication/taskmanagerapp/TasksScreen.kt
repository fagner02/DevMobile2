package com.example.myapplication.taskmanagerapp
...
fun TaskItem(task: Task, onToggleCompletion: () -> Unit, onDelete: () -> Unit) {
    val scale by animateFloatAsState(if (task.isCompleted) 1.05f else 1f, label = "scale")
...
fun AddTaskSection(onAddTask: (String, TaskCategory, TaskPriority) -> Unit) {
    ...
    Column {
        ...
        Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
            DropdownMenuBox("Categoria", TaskCategory.entries.map { it.name }) {
                selectedCategory = TaskCategory.valueOf(it)
            }
            DropdownMenuBox("Prioridade", TaskPriority.entries.map { it.name }) {
